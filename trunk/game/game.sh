...
if [ "$choice" == "1" ]; then
	buildoptions="-s -fstack-check -DNDEBUG -DMISSIONPACK -DQAGAME -D_SOF2 -fPIC -c"
	clear
	echo -e "Building a \e[00;36mPublic release build\e[00m" 
elif [ "$choice" == "2" ]; then
	buildoptions="-s -fstack-check -D_DEBUG -DDEBUG -DBUILDING_REF_GL -DDebugSoF2 -DMISSIONPACK -DQAGAME -D_SOF2 -D_BOE_DBG -fPIC -c"
	clear
