...
qboolean ConsoleCommand( void )
{
	...
	*/
	...
	// TODO (ajay#1#): Remove BEFORE 0.70 final release.
	// Boe!Man 3/11/13: Ifdef just to be safe.. This is a temporary command and cannot make it into >= 0.70 final!!
	#ifdef _DEBUG
	if (Q_stricmp (cmd, "sql_convertDbToBeta3") == 0 )
	{
		sqlite3			*db;
		sqlite3_stmt	*stmt;
		int				 rc;
		...
		Com_Printf("Opening database users(1).\n");
		// Boe!Man 3/11/13: Prepare database.
		if(!level.altPath){
		...
		if(rc){
			Com_Printf("Error: users database: %s\n", sqlite3_errmsg(db));
			return qtrue;
		}
		
		Com_Printf("Inserting queries to bypass the problem..\n");
		// Boe!Man 3/11/13: Convert databases.
		Com_Printf("Inserting update on sql master..\n");
		sqlite3_exec(db, "CREATE table temp_table1 as SELECT * FROM admins", NULL, NULL, NULL);
		sqlite3_exec(db, "DROP table admins", NULL, NULL, NULL);
		sqlite3_exec(db, "VACUUM", NULL, NULL, NULL);
		sqlite3_exec(db, "CREATE TABLE IF NOT EXISTS admins('IP' VARCHAR(24), 'name' VARCHAR(36) collate nocase, 'by' VARCHAR(36), 'level' INTEGER NOT NULL)", NULL, NULL, NULL);
		sqlite3_exec(db, "INSERT INTO admins SELECT * FROM temp_table1", NULL, NULL, NULL);
		sqlite3_exec(db, "DROP table temp_table1", NULL, NULL, NULL);
		
		sqlite3_exec(db, "CREATE table temp_table2 as SELECT * FROM passadmins", NULL, NULL, NULL);
		sqlite3_exec(db, "DROP table passadmins", NULL, NULL, NULL);
		sqlite3_exec(db, "VACUUM", NULL, NULL, NULL);
		sqlite3_exec(db, "CREATE TABLE IF NOT EXISTS passadmins('octet' VARCHAR(4), 'name' VARCHAR(36) collate nocase, 'by' VARCHAR(36), 'level' INTEGER NOT NULL)", NULL, NULL, NULL);
		sqlite3_exec(db, "INSERT INTO passadmins SELECT * FROM temp_table2", NULL, NULL, NULL);
		sqlite3_exec(db, "DROP table temp_table2", NULL, NULL, NULL);
		
		sqlite3_exec(db, "CREATE table temp_table as SELECT * FROM clanmembers", NULL, NULL, NULL);
		sqlite3_exec(db, "DROP table clanmembers", NULL, NULL, NULL);
		sqlite3_exec(db, "VACUUM", NULL, NULL, NULL);
		sqlite3_exec(db, "CREATE TABLE IF NOT EXISTS clanmembers('IP' VARCHAR(24), 'name' VARCHAR(36) collate nocase, 'by' VARCHAR(36))", NULL, NULL, NULL);
		sqlite3_exec(db, "INSERT INTO clanmembers SELECT * FROM temp_table", NULL, NULL, NULL);
		sqlite3_exec(db, "DROP table temp_table", NULL, NULL, NULL);
		
		sqlite3_exec(db, "VACUUM", NULL, NULL, NULL);
		sqlite3_close(db);
		
		// Boe!Man 3/11/13: Re-open db.
		if(!level.altPath){
			rc = sqlite3_open_v2("./users/users.db", &db, SQLITE_OPEN_READWRITE, NULL);
		}else{
			rc = sqlite3_open_v2(va("%s/users/users.db", level.altString), &db, SQLITE_OPEN_READWRITE, NULL);
		...
		Com_Printf("Testing if the update went correctly on users db..\n");
		Q_strncpyz(pass, g_sadminPass.string, sizeof(pass));
		if(sqlite3_exec(db, va("INSERT INTO admins (IP, name, by, level) values ('127.0.0.1', 'Boe', 'RCON-TEST', '5')"), 0, 0, 0) != SQLITE_OK){
			Com_Printf("Error: users database: %s\n", sqlite3_errmsg(db));
			sqlite3_close(db);
			return qtrue;
		}else{
			Com_Printf("Inserted 'Boe' (ip: '127.0.0.1') into the admins table.\n");
		}
		if(sqlite3_exec(db, va("INSERT INTO passadmins (octet, name, by, level) values ('h4x', 'L00na', 'RCON-TEST', 4)"), 0, 0, 0) != SQLITE_OK){
			Com_Printf("Error: users database: %s\n", sqlite3_errmsg(db));
			sqlite3_close(db);
			return qtrue;
		}else{
			Com_Printf("Inserted 'L00na' (ip: 'h4x') into the passadmins table.\n");
		}
		if(sqlite3_exec(db, va("INSERT INTO clanmembers (IP, name, by) values ('41.41.41.41', 'Sh0ke-n0b', 'RCON-TEST')"), 0, 0, 0) != SQLITE_OK){
			Com_Printf("Error: users database: %s\n", sqlite3_errmsg(db));
			sqlite3_close(db);
			return qtrue;
		}else{
			Com_Printf("Inserted 'Sh0ke-n0b' (ip: '41.41.41.41') into the clanmembers table.\n");
		}
		
		sqlite3_close(db);
		
		// Boe!Man 3/11/13: Check with the embedded funcs.
		if(!Boe_checkAdmin("127.0.0.1", "Boe")){
			Com_Printf("^1Error: ^7No success checking 127.0.0.1 - Boe\n");
			sqlite3_close(db);
			return qtrue;
		}
		if(!Boe_checkAdmin("127.0.0.1", "boe")){
			Com_Printf("^1Error: ^7No success checking 127.0.0.1 - boe. I.E. -> The update didn't do anything.\n");
			sqlite3_close(db);
			return qtrue;
		}
		if(g_passwordAdmins.integer){
			if(!Boe_checkPassAdmin("h4x", "L00na", pass)){
				Com_Printf("^1Error: ^7No success checking h4x - L00na.\n");
				sqlite3_close(db);
				return qtrue;
			}
			if(!Boe_checkPassAdmin("h4x", "l00na", pass)){
				Com_Printf("^1Error: ^7No success checking h4x - l00na. I.E. -> The update didn't do anything.\n");
				sqlite3_close(db);
				return qtrue;
			}
		}
		if(!Boe_checkClanMember("41.41.41.41", "Sh0ke-n0b")){
			Com_Printf("^1Error: ^7No success checking 41.41.41.41 - Sh0ke-n0b.\n");
			sqlite3_close(db);
			return qtrue;
		}
		if(!Boe_checkClanMember("41.41.41.41", "sh0ke-n0b")){
			Com_Printf("^1Error: ^7No success checking 41.41.41.41 - sh0ke-n0b. I.E. -> The update didn't do anything.\n");
			sqlite3_close(db);
			return qtrue;
		}
		...
		Com_Printf("Opening database aliases(2).\n");
		// Boe!Man 3/11/13: Prepare database.
		if(!level.altPath){
		...
		if(rc){
			Com_Printf("Error: aliases database: %s\n", sqlite3_errmsg(db));
			return qtrue;
		}
		
		Com_Printf("Inserting queries to bypass the problem..\n");
		Com_Printf("Inserting update on sql master..\n");
		sqlite3_exec(db, "CREATE table temp_table as SELECT * FROM aliases_names", NULL, NULL, NULL);
		sqlite3_exec(db, "DROP table aliases_names", NULL, NULL, NULL);
		sqlite3_exec(db, "VACUUM", NULL, NULL, NULL);
		sqlite3_exec(db, "CREATE TABLE IF NOT EXISTS aliases_names('ID' INTEGER, 'name' varchar(36) collate nocase)", NULL, NULL, NULL);
		sqlite3_exec(db, "INSERT INTO aliases_names SELECT * FROM temp_table", NULL, NULL, NULL);
		sqlite3_exec(db, "DROP table temp_table", NULL, NULL, NULL);
		sqlite3_exec(db, "VACUUM", NULL, NULL, NULL);
		
		sqlite3_close(db);
		
		Com_Printf("Testing if the update went correctly on aliases db..\n");
		Boe_addAlias("172.16.14.14", "SomeRandomer");
		
		// Boe!Man 3/11/13: Check with the embedded funcs.
		if(!Boe_checkAlias("172.16.14.14", "SomeRandomer")){
			Com_Printf("^1Error: ^7No success checking 172.16.14.14 - SomeRandomer.\n");
			sqlite3_close(db);
			return qtrue;
		}
		if(!Boe_checkAlias("172.16.14.14", "somerandomer")){
			Com_Printf("^1Error: ^7No success checking 172.16.14.14 - somerandomer. I.E. -> The update didn't do anything.\n");
			sqlite3_close(db);
			return qtrue;
		}
		Com_Printf("\nSuccess checking aliases.db!\n\n");
		
		Com_Printf("The convert seems to be overall succesfull!\n");
		return qtrue;
	}
	#endif
